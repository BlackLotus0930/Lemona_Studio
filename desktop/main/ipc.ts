...
export function setupIPC() {
  ...
  ipcMain.handle('document:create', async (_, title: string, folder?: 'library' | 'project' | 'worldlab') => {
    try {
  ...
  ipcMain.handle('document:uploadFile', async (_, filePath: string, fileName: string, folder: 'library' | 'project' | 'worldlab', projectId?: string) => {
    try {
  ...
  ipcMain.handle('document:updateFolder', async (_, id: string, folder: 'library' | 'project' | 'worldlab') => {
    try {
...
Rewritten text:`
      ...
      if (activeProfile?.type === 'custom-openai') {
        ...
        const msg = await openaiService.chat(
          apiKey,
          prompt,
          undefined,
          undefined,
          undefined,
          activeProfile.chatModel || 'gpt-4.1-nano',
          undefined,
          undefined,
          googleApiKey,
  ...
  ipcMain.handle('project:setCover', async (_, projectId: string) => {
    try {
    ...
    try {
      ...
      if (!storedFileName.toLowerCase().endsWith('.pdf')) {
        try {
          ...
          const findPdfNode = (node: any): boolean => {
            if (node?.type === 'pdfViewer') return true
  ...
  ipcMain.handle('library:indexAll', async (_, geminiApiKey?: string, openaiApiKey?: string, onlyUnindexed?: boolean) => {
    try {
  ...
  ipcMain.handle('integration:addSource', async (_, projectId: string, sourceType: 'rss' | 'github' | 'gitlab' | 'slack' | 'notion' | 'quickbooks' | 'linear' | 'stripe' | 'sentry' | 'posthog' | 'metabase' | 'hubspot' | 'db-schema', config: Record<string, unknown>, displayName?: string) => {
    try {
  ...
  ipcMain.handle('integration:startOAuth', async (event, projectId: string, sourceType: 'github' | 'gitlab' | 'slack' | 'notion' | 'quickbooks') => {
    try {
  ...
  ipcMain.handle('integration:getOAuthConfigStatus', async (_, sourceType: 'github' | 'gitlab' | 'slack' | 'notion' | 'quickbooks') => {
    try {
  ...
  ipcMain.handle('integration:saveOAuthConfig', async (_, sourceType: 'github' | 'gitlab' | 'slack' | 'notion' | 'quickbooks', config: { clientId: string; clientSecret: string }) => {
    try {
  ...
  ipcMain.handle('integration:listGithubRepos', async (_, projectId: string, sourceId: string) => {
    try {
  ...
  ipcMain.handle('integration:getIndexedGithubRepos', async (_, projectId: string, sourceId: string) => {
    try {
  ...
  ipcMain.handle('integration:updateGithubRepos', async (_, projectId: string, sourceId: string, repos: string[]) => {
    try {
  ...
  ipcMain.handle('integration:listGitlabRepos', async (_, projectId: string, sourceId: string) => {
    try {
  ...
  ipcMain.handle('integration:getIndexedGitlabRepos', async (_, projectId: string, sourceId: string) => {
    try {
  ...
  ipcMain.handle('integration:updateGitlabRepos', async (_, projectId: string, sourceId: string, repos: string[]) => {
    try {
  ...
  ipcMain.handle('integration:listSlackChannels', async (_, projectId: string, sourceId: string) => {
    try {
  ...
  ipcMain.handle('integration:getIndexedSlackChannels', async (_, projectId: string, sourceId: string) => {
    try {
  ...
  ipcMain.handle('integration:updateSlackChannels', async (_, projectId: string, sourceId: string, channels: string[]) => {
    try {
  ...
  ipcMain.handle('integration:listNotionPages', async (_, projectId: string, sourceId: string) => {
    try {
  ...
  ipcMain.handle('integration:getIndexedNotionPages', async (_, projectId: string, sourceId: string) => {
    try {
  ...
  ipcMain.handle('integration:updateNotionPages', async (_, projectId: string, sourceId: string, pageIds: string[]) => {
    try {
  ...
  ipcMain.handle('integration:removeSource', async (_, projectId: string, sourceId: string) => {
    try {
  ...
  ipcMain.handle('integration:getSources', async (_, projectId: string) => {
    try {
  ...
  ipcMain.handle('integration:syncSource', async (_, projectId: string, sourceId: string, geminiApiKey?: string, openaiApiKey?: string) => {
    try {
  ...
  ipcMain.handle('integration:syncAll', async (_, projectId: string, geminiApiKey?: string, openaiApiKey?: string) => {
    try {
  ...
  ipcMain.handle('library:search', async (_, query: string, projectId: string, geminiApiKey?: string, openaiApiKey?: string, fileIds?: string[], k?: number) => {
    try {
  ...
  ipcMain.handle('library:migrate', async (_, geminiApiKey?: string, openaiApiKey?: string) => {
    try {
  ...
  ipcMain.handle('settings:saveSmartIndexing', async (_, enabled: boolean) => {
    try {
  ...
  ipcMain.handle('settings:saveAiProviderProfile', async (_, profile: any) => {
    try {
  ...
  ipcMain.handle('settings:removeAiProviderProfile', async (_, profileId: string) => {
    try {
  ...
  ipcMain.handle('settings:setActiveAiProviders', async (_, active: { chatProviderId?: string; embeddingProviderId?: string }) => {
    try {
  ...
  ipcMain.handle('version:createCommit', async (_, projectId: string, documentSnapshots: any[], parentId?: string | null) => {
    try {
  ...
  ipcMain.handle('version:getCommits', async (_, projectId: string) => {
    try {
      if (!projectId || typeof projectId !== 'string') {
        throw new Error('Invalid projectId provided')
      }
  ...
  ipcMain.handle('version:getCommit', async (_, projectId: string, commitId: string) => {
    try {
  ...
  ipcMain.handle('version:getHeadCommit', async (_, projectId: string) => {
    try {
      if (!projectId || typeof projectId !== 'string') {
        throw new Error('Invalid projectId provided')
      }
  ...
  ipcMain.handle('version:restoreCommit', async (_, projectId: string, commitId: string) => {
    try {
